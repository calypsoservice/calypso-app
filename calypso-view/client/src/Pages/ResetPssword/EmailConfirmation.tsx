...
export const EmailConfirmationReset = () => {
    return (
...
const EmailConfirmationForm = () => {
    ...
    const submitVerificationCode = async () => {
        if (digitFirst.length == 1 && digitFirst.match(/^[0-9]$/) && digitSecond.length == 1 && digitSecond.match(/^[0-9]$/) && digitThird.length == 1 && digitThird.match(/^[0-9]$/) && digitFourth.length == 1 && digitFourth.match(/^[0-9]$/)) {
            ...
            axios.post(SERVER_URL + "/auth/recovery/password/check", data, {headers: HEADERS})
                .then(r => {
                    console.log(r)
...
const EmailConfirmationBottom = () => {
    ...
    const resendCode = () => {
        axios.post(SERVER_URL + "/auth/resend/recovery/email", { email:  email}, {headers: HEADERS})
