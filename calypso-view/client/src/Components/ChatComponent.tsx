...
export const ChatBlock = () => {
    ...
    useEffect(() => {
        ...
        messages.forEach((ms: IMessageContext) => {
            if (ms.read == false && ms.admin == true) {
                idList.push(ms.id)
            }
        })
    ...
    useEffect(() => {
        if (!appContext.user?.messages) return;
        var idList: number[] = []
        appContext.user.messages.forEach((ms: IMessageContext) => {
            if (ms.read == false && ms.admin == true) {
                idList.push(ms.id)
            }
        })
    ...
    return (
        <div className={`chat-button${chatOpen ? ' closed' : ''}`} data-unread={unread != 0 ? String(unread) : ''}>
            ...
            <ChatComponent isOpen={chatOpen} setIsOpen={setChatOpen} messages={messages} setMessages={setMessages} />
        </div>
    )
}


export const ChatComponent = ({ isOpen, setIsOpen, messages, setMessages }: { isOpen: boolean, setIsOpen: any, messages: IMessageContext[] | undefined, setMessages: any }) => {
    ...
    useEffect(() => {
        if (appContext.user) {
            setMessages(appContext.user.messages)
            setUpdate(update + 1)
            messagesWithDateWrapper(appContext.user.messages);
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            axios.post(SERVER_URL + '/message/get/by/user', { id: appContext.user?.id }, { headers: { ...HEADERS, Authorization: `${sessionStorage.getItem("auth-type")} ${sessionStorage.getItem("auth-token")}` } })
                .then(r => {
                    console.log(r)
                        ...
                        <div className="relative flex flex-col h-full">
                            <div className="chat-topbar px-4 py-2">
                                <div className="flex justify-between items-center gap-4">
                                    ...
                                    <div className="sm:hidden chat-close" onClick={() => setIsOpen(false)}>
...
const ChatBodyElement = ({ data }: { data: IMessageContext }) => {
    ...
    return (
        <li className={`flex m-0${data.admin == false ? ' justify-end' : ''}`}>
            <div className="inline-flex relative items-end max-w-[80%]">
                <div className={`m-0 min-w-6${data.admin == false ? ' ml-2 order-2' : ' mr-2'}`}>
                    <img className="rounded-full w-6 h-6" src="data:image/jpeg;base64," alt="" />
                </div>
                <div>
                    <div className="flex mb-2.5">
                        <div className={`relative px-5 py-3 chat-message${data.admin == false ? ' from-user' : ''}`}>
                            <p className="text-pretty text-xs">{data.text}</p>
                        </div>
                    </div>
                    <div className={`flex flex-wrap gap-2 font-medium itmes-center${data.admin == false ? ' justify-end' : ''}`}>
                        <small className="opacity-50 text-white text-xs">{date}</small>
                        <i className={`align-bottom text-xs read-status ri-check-double-line${data.read == false ? ' hidden' : ''}`} />
                    </div>
                </div>
            </div>
        </li>
